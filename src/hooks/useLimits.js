...
export default function useLimits(address, provider) {
  ...
  const refresh = useCallback(async () => {
    try {
      ...
      // Tenta via vaultService; se não houver, faz leitura direta do contrato.
      let limitRaw = 0n;
      const VAULT_ADDRESS =
        vaultService.getVaultAddress?.() || import.meta.env.VITE_VAULT_ADDRESS || "";

      if (typeof vaultService.dailyLimitUsd === "function") {
        limitRaw = await vaultService.dailyLimitUsd(provider).catch(() => 0n);
      ...
      const r = await vaultService.getUserLimit(provider, address).catch(() => ({ remainingUSD: 0n }));

      const limitUSD = Number(limitRaw ?? 0n);
      ...
      if (r?.remainingUSD != null) {
        ...
        remainingUSD = Number(BigInt(r.remainingUSD18) / (10n ** 18n));
