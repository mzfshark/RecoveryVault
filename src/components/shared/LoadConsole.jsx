  ...
  return (
    <div style={styles.wrap}>
      ...
      <div ref={boxRef} style={styles.box} role="log" aria-live="polite">
        {logs.length === 0 && <div style={{ opacity: 0.7 }}>initializing…</div>}
        {logs.map((l, i) => {
...
const styles = {
  ...
  progressOuter: { height: 3, background: "rgba(255,255,255,0.12)", margin: "0 10px", borderRadius: 3, overflow: "hidden" },
  progressInner: { height: "100%", background: "linear-gradient(90deg, #6ab7ff, #5ee7df)", transition: "width .25s ease" },
  dot: { display: "inline-block", width: 8, height: 8, borderRadius: 999, background: "#5ee7df", marginRight: 8, animation: "pulse 1s infinite" },
};
